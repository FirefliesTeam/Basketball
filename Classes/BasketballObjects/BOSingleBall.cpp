...
void BSingleBall::setPosition(cocos2d::Vec2 position) {
	sprite -> setPosition(position);
}


float BSingleBall::getLaunchingRotation(Vec2& start_touch_position, Vec2& current_touch_position) {
...
void  BSingleBall::startLaunching(cocos2d::Touch *touch) {
	if (!is_flying) {
...
void BSingleBall::setImpulse(cocos2d::Touch *touch) {
	if (!is_flying) {
		Vec2 current_touch_position = touch -> getLocationInView();
		Vec2 start_touch_position = touch -> getStartLocationInView();
...
void BSingleBall::launch(cocos2d::Touch *touch) {
	if (!is_flying) {
...
BSingleBall::~BSingleBall() {

}

void BSingleBall::serialize(std::ofstream& file){
	//file << 'b' << std::endl;
}

void BSingleBall::deserialize(std::ifstream& file){
	int x = 0;
	int y = 0;
